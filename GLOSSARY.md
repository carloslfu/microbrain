...
- **ablation** — removing one component and retraining, to measure what that
  ...
  Earned at rung 7.
- **activations** — the intermediate numbers flowing through the network for
...
- **argmax** — always picking the single highest-scoring option instead of
...
- **backpropagation / backprop** — see **autograd** and **backward pass**.
...
- **bigram** — a model that predicts the next token from the current token
...
- **BOS** — Beginning of Sequence, the one special token. Marks both "start
  ...
  Rung 0.
- **BPE** — byte-pair encoding: how production tokenizers build word-chunk
  tokens ("the", "ing") instead of single characters. Epilogue.
- **chain rule** — how a change flows through nested computations: multiply
...
- **closed-form** — answerable by a direct, exact formula, no trial-and-error
...
- **corpus** — the collection of documents a model trains on. Ours: 543 idea
  names. Rung 0.
...
- **derivative** — how sensitive one quantity is to a nudge in another. The
  gradient is the derivative of the loss with respect to each parameter.
  Rung 1.
- **diff** — a line-by-line comparison of two files showing exactly what
...
- **dot product** — multiply matching entries of two equal-length lists and
...
- **gist** — GitHub's format for sharing a single file. Both Karpathy
  originals live in gists, pinned in `reference/`.
- **GPT** — Generative Pre-trained Transformer: the family of
  next-token-predicting models behind ChatGPT and friends. This course builds
  one, complete, at 4,928 parameters.
- **GPU** — the massively parallel processor deep learning runs on; the
...
- **head / multi-head** — one attention lookup with its own query-key match
  ...
  cost. Rung 4.
- **heatmap** — a grid of shaded cells, darker = bigger value. Our shade
...
- **inference** — running a trained model to get outputs; no gradients, no
  learning. *Serving* is doing inference for many users at once. Rung 6.
- **init** — initialization: the starting values of all parameters, small
  random numbers here. Fixed by the seed, so runs reproduce. Rung 1.
- **instruments** — this course's word for the measurement code every rung
...
- **Laplace smoothing** — add one imaginary count to every cell so nothing
...
- **linear layer** — a matrix multiply: every output is a weighted sum of all
  ...
  Rung 1.
- **lm_head** — the final linear layer, turning the 16-number stream back
...
- **matmul / kernel** — matrix multiplication, and the hand-tuned GPU
...
- **MoE** — mixture of experts: a production MLP split into many "experts"
...
- **perplexity** — see **effective choices**.
...
- **rmsnorm** — rescale a vector so its typical entry size is 1 (unit
...
- **RoPE** — rotary position embeddings: production's replacement for the
  `wpe` table — positions as rotations, so there's no table to run out of.
  Epilogue.
- **rung** — one step of this course's ladder: a runnable file plus its
...
- **seed** — the number that fixes the random generator's choices; same seed
  → same "random" numbers → bit-identical runs. Ours is 42, which is why
  your logs can match the lessons exactly.
- **SFT / RL** — supervised fine-tuning (the same training loop pointed at
...
- **skip connections** — see **residual stream**.
- **slug** — a short, hyphenated, identifier-style name, like
...
- **tensor / scalar** — a scalar is one number; a tensor is a block of many
...
- **top-p** — production sampling's extra filter: sample only from the
...
- **transformer** — the architecture GPTs use: blocks of attention
