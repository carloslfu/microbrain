...
| scalar `Value`, one number each | tensors: the same graph over million-entry blocks | the 43× overhead from rung 2 vanishes when the bookkeeping spreads over blocks; GPUs then add ~10⁶× more arithmetic speed |
...
| 459 training docs, ~2 epochs, memorization gauge | trillions of tokens, *less than one* epoch | the regime flips: frontier models barely repeat data. Your overfitting lab is the tabletop model of the field's data-wall debate |
...
quantize them (store them in fewer bits). When a provider quotes you a price per token, the shape of that
